...
void addi(char *rs,char *rt, char *imm){//Effectue la commande ADDI
    int rs_d;
    int rt_d;
    int imm_d;
    int signe = 0;
...
void mult_(char *rs, char*rt){////Effectue la commande MULT et stoque les résultat dans LO et HI
    ...
    if((resultat>max) || (resultat<min)){//Teste si on a pas atteint la valeur max/min
        ...
        hi = resultat>>32;////Permet d'obtenir les 32 derniers bits de résultat
            if(lo!=0){//On modifie seulement si ça vos pas 0
                put_value_registre(32,lo);
            }
            if(hi!=0){
                put_value_registre(33,(int)hi);
            }
            
    }
}
void div_(char *rs, char*rt){
    int rs_d;//Effectue la commande DIV et stoque le résultat dans LO et le reste dans HI
    int rt_d;
    int resultat;
    rs_d = (int)bit_to_decimal(rs,0,0);
    rt_d = (int)bit_to_decimal(rt,0,0);
    if(get_value_registre(rt_d) == 0){
        printf("->DIV $%d, $%d\n",rs_d,rt_d);
        fatal("Division par 0");
    }else{
        resultat = (int)get_value_registre(rs_d) / (int)get_value_registre(rt_d);//Fait la division 
        put_value_registre(32,resultat);
        resultat = (int)get_value_registre(rs_d) % (int)get_value_registre(rt_d);//Calcul du reste 
        put_value_registre(33,resultat);
    }
...
void rotr(char *sa,char *rt, char *rd){//Effectue la commande ROTR
    ...
    if(rd_d ==0){
        ...
        resultat = ((get_value_registre(rt_d)>>sa_d)|(get_value_registre(rt_d)<<(32-sa_d)));//right shift de sa_d de la valeut puis Ou bit à bit de la valeur left sift de 32-sa_d de la valeur puis un & bit a bit pour mettre le tout sur 32 bits
        put_value_registre(rd_d,resultat);//Met la valeur dans le registre rd_d
    }
...
void bgtz(char *rs,char *offset,int mode){//Effectue la commande BGTZ
    ...
    add_tamp = ((int)offset_d<<2) + (get_pc());//On applique un <<2 a notre offset pour qu'il soit multiple de 4
...
void bne(char *rs,char *rt,char *offset,int mode){
    ...
    offset_d = (int)bit_to_decimal(offset,signe,0);
    add_tamp = ((int)offset_d<<2) + (get_pc());
    if((add_tamp<0)||(add_tamp%4!=0)){
        printf("->BNE $%d, $%d, %d\n",rs_d,rt_d,offset_d);
        fatal("L'adresse ciblée est incorecte");
    }else{
        if(get_value_registre(rs_d)!=get_value_registre(rt_d)){
            if(last_instrction()){
                modifier_pc(add_tamp);
            }else{
                execution(mode);
...
void j(char *adresse,int mode){
    ...
    ad_tamp = bit_to_decimal(adresse, 0,0);
    ad_tamp = ad_tamp<<2;
    bit(get_pc(),32,pc);
    bit(ad_tamp,32,ad_tamp_bit);
    for(int i=0;i<4;i++){
        ad_tamp_bit[i] = pc[i];
    }
    ad_tamp = bit_to_decimal(ad_tamp_bit,0,0);
    if(last_instrction()){
        modifier_pc(ad_tamp);
    }else{
        execution(mode);
...
void jr(char *rs,int mode){
    int rs_d;
    rs_d = (int)bit_to_decimal(rs,0,0);
    if(get_value_registre(rs_d)<0||(get_value_registre(rs_d)%4 !=0)){
        printf("->JR $%d\n",rs_d);
        fatal("L'adresse ciblée est incorecte");
    }else{
        if(last_instrction()){
...
void sw(char *base, char *rt, char *offset){
    ...
    adresse = get_value_registre(base_d) + offset_d;
    
    if((adresse%4!=0)||(adresse<0)){
        fatal("L'adresse ciblée est incorecte");
    }else{
        ajouter_donnée(get_value_registre(rt_d),adresse);
...
void lw(char *base, char *rt, char *offset){
    ...
    if(rt_d==0){
        ...
        if((adresse%4!=0)||(adresse<0)){
            ...
            put_value_registre(rt_d,obtenir_donnée(adresse));
        }
    }
}
...
void execution(int mode){
    ...
    get_instruction(instruction_hexa);
    convert_hexa_bit(instruction_hexa,mot_bit);
    if((mode==0)||(mode==1)){
        printf("%s\n",instruction_hexa);
    }
