   ...
   if(mode ==0){
      ...
      while(leave == 0){
         ...
         if(strcmp(commande, "\0")==0){//On test qu'on ai pas appuyé plusieur fois entrée
            ...
            fonctions(commande,mode,argv[2],registre);// appel la commande qui converti notre commande en hexa    
         }
      }
      ...
      while (fscanf(prog_file, "%[^\n]%*c", commande)!=EOF)// On lis notre fichier jusquà qu'i soit vide
      {
         ...
         if((commande[0] != '#')){// Teste si ce n'est pas uin commentire
            fonctions(commande,mode,argv[2],registre);// Appelle de la fonction qui converti notr commande en hexa
            if(mode == 1){
